...
(function () {
  ...
  // 4 thèmes mis en avant dans la card "Explore par thème" (le reste accessible via le bouton)
  const FEATURED_THEMES = ['argent', 'logement', 'travail', 'sante'];

  function renderThemesPreview() {
    const totalThemes = MODULES.length;
    const featured = FEATURED_THEMES.map(id => MODULES_INDEX[id]).filter(Boolean);
    return `
      <section class="themes-preview" aria-labelledby="themes-preview-label">
        <header class="themes-preview-head">
          ...
          <span class="themes-preview-count">${totalThemes} thèmes</span>
        </header>
        <p class="themes-preview-lead">Découvre les 9 thèmes de l'app et choisis ce qui t'intéresse.</p>
        <div class="themes-preview-grid">
          ${featured.map(m => `
            <a class="theme-mini" href="#/module/${m.id}" data-color="${m.color}">
              <span class="theme-mini-icon">${m.icon}</span>
              <strong>${escapeHtml(m.title)}</strong>
              <span>${escapeHtml(m.subtitle || '')}</span>
            </a>
          `).join('')}
        </div>
        <a class="themes-preview-cta" href="#/sujets">Voir les ${totalThemes} thèmes <span aria-hidden="true">→</span></a>
      </section>
    `;
  }
    ...
    main.innerHTML = `
      ...
      <footer class="app-footer">
        ...
        <a href="https://famillesdaccueil.mypixieset.com/" target="_blank" rel="noopener" class="signature-block">
          <img src="assets/icons/sfa-verviers.svg" alt="" class="signature-logo" />
        ...
        <p class="updated-date">Contenu mis à jour le 2 mai 2026.</p>
      </footer>
    `;
    injectResumeBanner();
  }
  ...
  function renderUrgence() {
    const calls = [
      { tel: '112', title: '112 — Urgences vitales', sub: 'Police, ambulance, pompiers · 24/7 · gratuit', primary: true },
      { tel: '1733', title: '1733 — Médecin de garde', sub: 'Soins courants en soirée et week-end' },
      { tel: '0702452452', title: '070 245 245 — Centre Anti-Poisons', sub: 'Intoxication, ingestion · 24/7 · gratuit' },
      { tel: '103', title: '103 — Écoute Enfants Ado', sub: 'Pour les moins de 20 ans · gratuit, anonyme' },
      { tel: '107', title: '107 — Télé-Accueil', sub: 'Quand ça ne va pas, parle à quelqu\'un · 24/7' },
      { tel: '080032123', title: '0800 32 123 — Prévention du suicide', sub: 'Gratuit · 24/7 · anonyme' },
      { tel: '080098100', title: '0800 98 100 — SOS Viol', sub: 'Gratuit · anonyme' },
      { tel: '080030030', title: '0800 30 030 — Violences conjugales', sub: 'Écoute et orientation' },
      { tel: '022275252', title: '02 227 52 52 — Infor-Drogues', sub: 'Gratuit · anonyme' }
    ];
    main.innerHTML = `
      <section class="urgence-page">
        ...
        ${calls.map(c => `
          <a class="urgence-card ${c.primary ? 'primary' : ''}" href="tel:${c.tel}">
  ...
  function renderMonEspace() {
    ...
    main.innerHTML = `
      ...
      <div class="tiles">
        ${tile('#/mes-documents', 'admin', '<svg viewBox="0 0 24 24" width="24" height="24"><path d="M9 12l2 2 4-4M7 4H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><rect x="8" y="2" width="8" height="4" rx="1" fill="none" stroke="currentColor" stroke-width="2"/></svg>', 'Mes documents', `${counts.docs} / ${counts.docsTotal}`)}
        ...
        ${tile('#/favoris', 'argent', '<svg viewBox="0 0 24 24" width="24" height="24"><path d="M12 21l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.18L12 21z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>', 'Mes favoris', counts.favoris ? `${counts.favoris} fiche${counts.favoris > 1 ? 's' : ''}` : 'Vide')}
        ${tile('#/urgence', 'urgence', ICONS.sos, 'Aide & urgences', 'Numéros utiles')}
      </div>

      <h2 class="section-label">À propos</h2>
      <article class="fiche">
        <p>MonAvenir aide les jeunes à comprendre leurs droits, gérer l'administratif, et trouver de l'aide. <strong>Sans compte. Sans pub. Sans tracking.</strong></p>
  ...
  function renderAPropos() {
    main.innerHTML = `
      ...
      <h1 class="page-title">MonAvenir, c'est quoi&nbsp;?</h1>
      ...
      <section class="apropos-card">
        ...
        <p>Trop d'infos utiles pour les jeunes majeur·es sont cachées dans du jargon administratif. MonAvenir rassemble l'essentiel : devenir majeur·e, papiers, argent, logement, travail, études, santé, vie quotidienne, urgences. Tu lis, tu prends ce qui t'aide, tu refermes.</p>
      </section>

      <section class="apropos-card">
        <div class="apropos-initiative">
          <img src="assets/icons/sfa-verviers.svg" alt="" class="apropos-logo" />
          <div>
        ...
        <p>MonAvenir est née au Service Familles d'Accueil de Verviers, qui accompagne depuis des années les jeunes en placement et leurs familles d'accueil. Sa porte te reste ouverte après tes 18 ans : pour une question, un doute, une écoute.</p>
      ...
      <p class="apropos-meta">Contenu mis à jour le 2 mai 2026. Vérifié à partir des sites publics belges.</p>
    `;
  }
  ...
  function updatePageMetadata() {
    ...
    document.title = titleText ? `${titleText} — MonAvenir` : 'MonAvenir — Tu n\'es pas seul·e';
    main.focus();
  }
